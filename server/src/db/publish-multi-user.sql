-- Preflight checks: ensure no duplicates or null user_id values
select user_id, count(*) from public.profile group by user_id having count(*) > 1;
...
-- Migration: multi-user hardening and RLS policies
...
-- Optional: expand document types to match API
